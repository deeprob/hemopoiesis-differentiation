...
Questions to answer: CMP vs ERY
...
get_pca_rep = function(cts){
  rv = rowVars(as.matrix(cts))
  select = order(rv, decreasing = TRUE)[1:15000]
  pca = prcomp(t(cts[select, ]), scale. = TRUE)
  
  
  
  condition = factor(c(rep(c(cell_line_a), 2), rep(c(cell_line_b),2)))
  colType <- c("forestgreen", "red2")[condition]
  pchType <- c(18, 16)[condition]
  plot(pca$x, col=colType, pch=pchType, cex = 2.0)
  legend(
    "topleft",
    bty = "n",
    c(cell_line_a, cell_line_b),
...
gen_volcano_deseq = function(deseq_res){
  tab = data.frame(logFC = deseq_res$log2FoldChange, negLogPval = -log10(deseq_res$padj))
  par(mar = c(5, 4, 4, 4))
  plot(tab, pch = 16, cex = 0.6, xlab = expression(log[2]~fold~change),ylab = expression(-log[10]~pvalue), main="DESeq2 Volcano Plot")
  lfc = 2
...
gen_volcano_limma = function(limma_res){
  tab = data.frame(logFC = limma_res$logFC, negLogPval = -log10(limma_res$adj.P.Val))
  par(mar = c(5, 4, 4, 4))
  plot(tab, pch = 16, cex = 0.6, xlab = expression(log[2]~fold~change),ylab = expression(-log[10]~pvalue), main="Limma-voom Volcano Plot")
  lfc = 2
  pval = 0.001## Selecting interest genes
  signGenes = (abs(tab$logFC) > lfc & tab$negLogPval > -log10(pval))
  ## Identifying the selected genes
